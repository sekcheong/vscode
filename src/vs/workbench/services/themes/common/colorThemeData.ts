...
export class ColorThemeData implements IWorkbenchColorTheme {
	...
	private themeSemanticHighlightSupport: boolean;
	...
	private constructor(id: string, label: string, settingsId: string) {
		...
		this.isLoaded = false;
		this.themeSemanticHighlightSupport = false;
		this.customSemanticHighlightSupport = false;
	}

	get supportsSemanticHighlighting(): boolean {
		return (this.customSemanticHighlightSupport !== undefined) ? this.customSemanticHighlightSupport : this.themeSemanticHighlightSupport;
	}
	...
	public setCustomTokenColors(customTokenColors: ITokenColorCustomizations) {
		...
		if (types.isObject(themeSpecificTokenColors)) {
			...
			this.customSemanticHighlightSupport = customTokenColors.supportsSemanticHighlighting;
		}
	...
	private load(extensionResourceLoaderService: IExtensionResourceLoaderService): Promise<void> {
		...
		return _loadColorTheme(extensionResourceLoaderService, this.location, result).then(_ => {
			...
			this.themeSemanticHighlightSupport = result.supportsSemanticHighlighting;
		});
	}
...
function _loadColorTheme(extensionResourceLoaderService: IExtensionResourceLoaderService, themeLocation: URI, result: { textMateRules: ITextMateThemingRule[], colors: IColorMap, stylingRules: TokenStylingRule[] | undefined, supportsSemanticHighlighting: boolean }): Promise<any> {
	if (resources.extname(themeLocation) === '.json') {
		return extensionResourceLoaderService.readExtensionResource(themeLocation).then(content => {
			...
			return includeCompletes.then(_ => {
				...
				result.supportsSemanticHighlighting = result.supportsSemanticHighlighting || contentValue.supportsSemanticHighlighting;
